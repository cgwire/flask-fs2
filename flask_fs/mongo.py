...
class ImageReference(FileReference):
    ...
    def save(self, file_or_wfs, filename=None, bbox=None, overwrite=None):
        ...
        if self.max_size:
            ...
            file_or_wfs.seek(0)
            if resized:
                self.original = self.fs.save(
                    file_or_wfs, name("original"), **kwargs
                )
    ...
    def full(self, external=False):
        ...
        return (
            self.fs.url(self.filename, external=external)
            if self.filename
            else None
        )
    ...
    def rerender(self):
        ...
        with self.fs.open(self.original, "rb") as f_img:
            img = io.BytesIO(
                f_img.read()
            )  # Store the image in memory to avoid overwritting
...
class FileField(BaseField):
    ...
    def __init__(
        self, fs=None, upload_to=None, basename=None, *args, **kwargs
    ):
        self.fs = fs
        self.upload_to = upload_to
        self.basename = basename
...
class ImageField(FileField):
    ...
    def __init__(
        self, max_size=None, thumbnails=None, optimize=None, *args, **kwargs
    ):
        self.max_size = max_size
    ...
    def proxy(self, **kwargs):
        return super(ImageField, self).proxy(
